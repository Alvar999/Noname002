        ...
        for(new i = 0; i < MAX_PLAYERS; i++)
        {
                if(IsPlayerConnected(i)) //Maybe you can add a log in checker
                {
                        if(GetPlayerSurfingVehicleID(i) != INVALID_VEHICLE_ID) //You can also add something like if player is not admin duty
                        {
                                ...
                                if(floatround(rtn*100*1.61,floatround_round) > MAX_SPEED_ANTICS && ! (IsABoat(GetPlayerSurfingVehicleID(i))) && GetVehicleModel(GetPlayerSurfingVehicleID(i)) != 406 && GetVehicleModel(GetPlayerSurfingVehicleID(i)) != 578 && GetVehicleModel(GetPlayerSurfingVehicleID(i)) != 433)
                                {
                                                new string[128];
                                                if(PlayerSurfing[i] == 0)
                                                {
                                                    new sz_playerName[MAX_PLAYER_NAME], i_pos;
                                                        ...
                                                        format(string, sizeof(string), "* %s Da Bi Te Ra Khoi Xe Vi Dung Tren Noc Xe.", sz_playerName);
